...
dshw <- function(y, period1=NULL, period2=NULL, h=period1, alpha=NULL, beta=NULL, gamma=NULL, omega=NULL, phi=NULL, lambda=NULL, armethod=TRUE, model = NULL)
{
...
dshw.mse <- function(par, y, period1, period2, pars)
{
  ...
  if(max(pars) > 0.99 | min(pars) < 0 | pars[5] > .99)
    ...
    return(dshw(y, period1, period2, h=period1, pars[1], pars[2], pars[3], pars[4], pars[5], armethod=(abs(pars[5]) >1e-7))$model$mse)
}
